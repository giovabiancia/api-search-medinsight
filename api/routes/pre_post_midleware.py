...
def print_info_before():
    ...
    # loggerManager.logger.info(f'json: {request.json} ')
...
@bp.before_app_request
def connect_to_db():
    # if current_app.config.get('ENV') != 'DOCUMENTATION':
    loggerManager.logger.info("Connecting to db")
...
def close_db_if_open(response):
    ...
    if connected:
        loggerManager.logger.info("Closing db")
